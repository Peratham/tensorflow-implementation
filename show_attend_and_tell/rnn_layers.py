...
def attention_forward(X, prev_h, params, hyper_params):
    ...
    Returns:
    - context: output data (context vector) of shape (N, D) for soft attention
    """
    ...
    alpha = tf.expand_dims(alpha, 2)  # (N, L, 1)
    context = tf.reduce_sum(X * alpha, 1)  # (N, D)
    return context
...
def lstm_step_forward_with_attention(X, features, prev_h, prev_c, params, hyper_params):
    ...
    context = attention_forward(features, prev_h, params, hyper_params)
...
def lstm_forward(X, features, h0, params, hyper_params):
    """
    Inputs:
    ...
    for t in range(T):
        next_h, next_c = lstm_step_forward_with_attention(X[:, t, :], features, prev_h, prev_c, params, hyper_params)
        h_list.append(next_h)  # tensor flow doesn't provide item assignment such as h[:,t,:] = next_h
        prev_h = next_h
        prev_c = next_c
    h = tf.transpose(tf.pack(h_list), (1, 0, 2))
...
def temporal_affine_forward(X, W, b, hyper_params):
    """
    Inputs:
